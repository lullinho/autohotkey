...
MsgBox % Dictonary.HasKey("One") ; retorna 1 se a key existe, 0 se não existe
...
; checar se existe um valor presente
for k, v in Dictonary
	if(v=2){
		MsgBox % "Found value" v " at this key " k
		
}

; exemplo lista de ordens

Orders:={"Joe": "Burger", "John": "Taco", "Susan":"Steak"}
For Person, Meal in Orders
	data.=Person " wants " Meal "`n"
DebugWindow(data, Clear:=1, LineBreak:=1, Sleep:=500, AutoHide:=0)
MsgBox % Orderns["Joe"] ; acessar o que joe quer, key joe

; exemplo produtos e ordens, fazer soma do valor das ordens
Products:={"Aspirina": 4.40, "Advil": 5.20, "Excedrin": 3}
Orders:=["Aspirina", "Advil"]
sum:=0
For, k,v in Orders
	sum:=sum+prods[v]

MsgBox % Sum ; quer saber o quanto o pedido custou
